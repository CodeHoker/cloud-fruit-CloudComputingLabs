...
void setCommand(RedisClient *c) {
    ...
    if(c->argc < 3)
        ...
        strResult = "+OK\r\n";
        // 实时更新
        s->sendMessage(s->heartBeat_send("SYN", false, s->oldCmds.size()), "follower");
    }
...
void beMyCommand(RedisClient* c) {
    ...
    if(c->argv[2] == "leader") {
        ...
        for(auto it : s->followerClients) {
            if(c == it.second)
                continue;
            else if(c->serverPort == it.second->serverPort && c->addr.sin_addr.s_addr == it.second->addr.sin_addr.s_addr) {
                ret[6] = to_string(stoi(ret[6])-1);
                continue;
            }
            ret.emplace_back("$");
...
void otherFollowerCommand(RedisClient* c) {
    ...
    for(int i = 0; i < size_otherFollowers; i++) {
        ...
        // 有个问题,如果自己是第二次slaveof的话,那么在otherfollower会把自己给连上
